...
### For sample LUM4347 from the paper, use Monte Carlo to get the age error.

#setwd('D:/1-Blessberg cave speleothem/new modelling of dose rate with Sa system/3 segments')
...
runs <- 300  ## run 300 simulations. It takes some time..
U_simu  <- rnorm(runs,mean=U,sd=errU)
r0_simu <- rnorm(runs,mean=r0,sd=errr0)
Sa_simu <- rnorm(runs,mean=Sa,sd=errSa)
De_simu <- rnorm(runs,mean=Demeasure,sd=errDe)
...
#a.Th230<-lamda.Th230*N.Th230.initial
#a.Th230
...
dev<-min(abs(Demeasure - dataset$De))
    
Dedata<-subset(dataset,abs(Demeasure - dataset$De) == dev)
#Dedata
