##Dose rate calculation for coarse grain quartz, etched
##No need to consider alpha dose rate.
...
##Input data into the csv file - Template Coarse Quartz.csv

##The units of U and Th is ppm, the unit of K (not K2O) is %.
...
Dr<-read.csv("Template coarse-grain Quartz.csv",header = TRUE)
#print(Dr) ###not necessary, just to make sure the 'read in' is successful

result_filename<-"Dr results quartz coarse grain - etched 20perRnloss.csv"
...
etch.depth<-10      #the thickness of the layer of quartz removed by HF etching (unit um). In other word, the reduce of the radius. 
#####################
...
########################################################## beta dose rate calculation ################################# 
  ...
  absorb.K=-3.38E-4+3.78E-4*grain
  absorb.K.err=(3.78E-4*grain.max-3.78E-4*grain.min)/2

  f_absorb_U <- function(x) {
    y <- 0.01410+0.6065*(1-exp(-x/1185.92))+0.03485*(1-exp(-x/38.82)) #fitted in Origin
    return(y)
  }
  ...
  f_absorb_Th <- function(x) {
    y <- 0.01995+0.5614*(1-exp(-x/1138.48))+0.10080*(1-exp(-x/90.65)) #fitted in Origin
    return(y)
  }
  ...
  #output the parameters
   ...
   ########################################################## beta dose rate calculation ################################# 
    ...
    beta.K.error=beta.K*sqrt((K.error/K)^2+(etch.factor*absorb.K.err)^2/(1-etch.factor*absorb.K)^2+(conversion.beta.K.err/conversion.beta.K)^2)
    ...
    beta.U.error=beta.U*sqrt((U.error/U)^2+(etch.factor*absorb.U.err)^2/(1-etch.factor*absorb.U)^2+(conversion.beta.U.preRadon.err^2+(1-Radon.loss)^2*conversion.beta.U.afterRadon.err^2)/(conversion.beta.U.preRadon+(1-Radon.loss)*conversion.beta.U.afterRadon)^2)
    cat('  beta.U=',beta.U,'+-',beta.U.error,'Gy','\n')
    
    beta.Th=Th*(1-etch.factor*absorb.Th)*conversion.beta.Th
    beta.Th.error=beta.Th*sqrt((Th.error/Th)^2+(etch.factor*absorb.Th.err)^2/(1-etch.factor*absorb.Th)^2+(conversion.beta.Th.err/conversion.beta.Th)^2)
    ...
    ########################################### total dose rate ################################################################
   Doserate=cosmic.ray+beta+gamma
   Doserate.error=sqrt(cosmic.error^2+beta.error^2+gamma.error^2)
   Dr[i,24]=Doserate
   ...
   ######################################
    if(i==nrow(Dr)) 
     ...
     print(Dr[,c(1,18,19,20,21,22,23,24,25)])
      }
}
