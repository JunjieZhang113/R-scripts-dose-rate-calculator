##Dose rate calculation for coarse grain quartz, not etched 
##need to consider alpha dose rate.
...
result_filename<-"Dr results quartz coarse grain - no etching.csv"
...
alpha.factor<-0.04     #It is the a-value (k3.7). it will be converted to keff, which is used in the alpha dose rate calculation below. keff = 0.80*K3.7 for U, and 0.86*k3.7 for Th (Zimmerman 1971, page 313 in Aitken 1985)
alpha.factor.err<-0.01 #Error of alpha factor, here it is set as 0.01.
########################
#####################
...
######################################### cosmic ray dose rate calculation #############################
  ...
  # calculate the alpha-dose attenuation factor.The double exponential function is fitted from Brenann (1991).
  grain<-(grain.min+grain.max)/2
  
  f_attenuation_alphaU<-function(x){
    y<- 0.8144*exp(-x/22.5189)+0.2616*exp(-x/109.0385)+0.03279
    return(y)
  }
  ...
  attenuation.alphaU.err
  
  f_attenuation_alphaTh<-function(x){
    y<- 0.8558*exp(-x/30.4128)+0.2555*exp(-x/280.9105)-0.03402
    return(y)
  }
...
########################################################## beta absorption factor################################# 
  ...
  #output the parameters
   ...
   cat('*******************************Sample ID: ',sampleID, '*******************************************************************','\n', 
    ...
    'Alpha attenuation factor for U=',attenuation.alphaU,'+-',attenuation.alphaU.err,"\n",
   ...
   # Note that 0.80 for U and 0.86 for Th are the coefficients from k3.7 to keff.
